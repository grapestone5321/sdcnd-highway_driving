...
int NextWaypoint(double x, double y, double theta, const vector<double> &maps_x, const vector<double> &maps_y, const vector<double> &maps_dx, const vector<double> &maps_dy)
{
...
vector<double> getFrenet(double x, double y, double theta, const vector<double> &maps_x, const vector<double> &maps_y, const vector<double> &maps_dx, const vector<double> &maps_dy)
{
...
int main() 
{
  ...
  h.onMessage([&ref_vel, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy, &lane, &SPEED_LIMIT, &match_speed](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) 
{
      ...
      if (s != "") 
      {
        ...
        if (event == "telemetry") 
        {
          ...
          else if (ref_vel < SPEED_LIMIT && !match_speed)
          {
            ...
            // Caluculate how to break up spline points
            ...
            for (int i = 1; i <= 50 - previous_path_x.size(); i++)
            {
              ...
              // rotate back to nomal after rotating it eariler
